...
단, `.useFPS()`와 `useTPV()`는  아바타에 `.setFollowingCamera()` 메소드가 적용되어 있어야 작동합니다.  
...
스크립트 내에서 코드를 통해 카메라의 매개변수(`.fov`, `.far`, `.near`)를 변경하셨다면, 반드시 `.updateProjectionMatrix ()`를 사용해야 변경된 값이 즉시 적용됩니다.  
