...
원본 객체의 모든 속성을 복사하여 새로운 객체를 생성합니다. 이는 객체의 기하학적 모양, 재질, 위치, 회전, 크기 등을 포함합니다.  
...
`object.traverse ( callback : Function )` : 정의되지 않음
...
x,y,x - 벡터의 x, y, x값을 선택적으로 지정할 수 있습니다.  

object를 특정 지점(벡터)을 향하도록 회전시키는 메서드입니다.  
