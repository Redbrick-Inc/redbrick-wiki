...
[`Object.position.set(x, y, x)`](../built-in/three/vector3#set)을 사용하면 월드 내의 특정 좌표로 오브젝트를 즉시 이동시킬 수 있습니다.  
...
PLAYER.body.needUpdate = true; //PLAYER 또는 Physics Body 속성이 활성화 되어있는 오브젝트는 위치 변경 후 이 코드를 실행해주어야 위치 변경이 반영됩니다
```
...
obj.position.z = PLAYER.position.z; //오브젝트의 z 좌표 값만 플레이어의 z 좌표와 동일하게 설정
PLAYER.position.y = 100; //플레이어의 높이를 100으로 변경
...
[`Object.position.set(x, y, x)`](../built-in/three/vector3#set) 메소드를 다음과 같이 사용하면 상대 좌표 이동을 구현할 수 있습니다.  
...
obj.position.x += 10; //오브젝트의 현재 좌표에서 +10 만큼 이동 (y, z 좌표는 동일)
obj.position.z = PLAYER.position.z - 10; //플레이어의 좌표에서 z - 10의 위치로 이동 (x, y 좌표는 동일)
PLAYER.position.y += 10; //플레이어의 높이를 현재 높이 기준 +10 만큼 변경
...
obj.go(0, 5, 0); //obj의 높이를 현재 높이 기준 +5만큼 이동시킵니다.
