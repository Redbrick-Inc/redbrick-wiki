...
<br/>
<center>
![](../../../images/use-clone-1.gif)오브젝트 복제
</center>

오브젝트의 복제는 [`.clone()`](../script/built-in/three/object3d#clone) 또는 [`.cloneWithMethods()`](../script/api/object3d#clonewithmethods)를 사용해 수행할 수 있습니다.  
...
이 두 메소드의 차이는 [확장된 Ojbect3D API의 메소드](../script/api/object3d.mdx)들을 사용할 수 있냐 없냐 입니다.  

`.clone()`은 확장된 Object3D의 메소드를 사용할 수 없습니다.  
`.cloneWithMethods()`는 사용할 수 있지만, 복제된 오브젝트는 Physics의 Body속성이 비활성화 되어있으므로 [`onCollide()`](../script/api/object3d#oncollide) 메소드는 사용할 수 없습니다.  

## .clone()

```js showLineNumbers filename=".clone()" copy
...
```js showLineNumbers filename=".cloneWithMethods()" copy
const obj = WORLD.getObject("obj");
const clone_btn = GUI.getObject("clone_btn");
let clone_obj_list = []; //복제한 오브젝트를 담을 배열
...
<center>
<br/>
![](../../../images/use-clone-2.gif)복제한 오브젝트로 충돌 검사
</center>
