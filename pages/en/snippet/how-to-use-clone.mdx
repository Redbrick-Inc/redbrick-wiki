...
<br/>
<center>
![](../../../images/use-clone-1.gif)Object Cloning
</center>

Object cloning can be performed using [`.clone()`](../script/built-in/three/object3d#clone) or [`.cloneWithMethods()`](../script/api/object3d#clonewithmethods).  
...
The difference between these two methods lies in their ability to utilize methods from the [extended Object3D API](../script/api/object3d.mdx).  

`.clone()` does not allow the use of methods from the extended Object3D API.  
`.cloneWithMethods()` allows their use, but cloned objects have the Physics Body attribute disabled, so the [`onCollide()`](../script/api/object3d#oncollide) method cannot be used.  

## .clone()

```js showLineNumbers filename="Tracking_player" copy
...
```js showLineNumbers filename="Distance_calculation" copy
