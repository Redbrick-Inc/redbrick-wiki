...
however, `.useFPS()` and `useTPV()` only work if the `.setFollowingCamera()` method is applied to the avatar.  
...
If you have modified the parameters of the camera (`.fov`, `.far`, `.near`) through code within the script, you must use `.updateProjectionMatrix()` to immediately apply the changed values.  
